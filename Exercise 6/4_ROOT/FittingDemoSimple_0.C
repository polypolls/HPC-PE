...
void FittingDemoSimple_0()
{
  ...
  // Create the TCanvas window used for drawing and displaying root objects
  TCanvas *c1 = new TCanvas("c1", "Fitting Demo", 10, 10, 700, 500);
  // histogram is created
  TH1F *histo = new TH1F("histo", "Quadratic Background", 60, 0, 3);

  // Using a random number generator we fill the histogram with 1000000 random numbers
  TRandom3 rand;
  for (int i = 0; i < 1000000; i++)
    histo->Fill((rand.Uniform() + rand.Uniform()) * 1.5); // fill with triangular distribution
