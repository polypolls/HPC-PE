...
int main()
{
  ...
  for (int io = 0; io < NIterOut; io++)
  {
    for (int i = 0; i < NVectors; i++)
    {
      // Vc float vectors which store the coefficients of the equation
      float_v aV;
      float_v bV;
      float_v cV;

      // We use this loop to copy the coefficients from dataSIMD1 to Vc float vectors
      ...
      // Using float Vc vectors we calculate the discriminant of the quadr. equation
      const float_v det = bV * bV - 4 * aV * cV;
      // Calculating the square roots with the quadratic formula and the Vc vectors
      float_v xV = (-bV + sqrt(det)) / (2 * aV);

      // we copy the output data
  ...
  for (int io = 0; io < NIterOut; io++)
    ...
    { // reinterpreting the elements as Vc float vector types
